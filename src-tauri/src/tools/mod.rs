...
pub fn register_all(registry: &mut crate::tool_registry::ToolRegistry) {
    // Tools registered here as they are implemented.
    // Each task in this plan adds a register call here.
    let _ = registry; // suppress unused warning until first tool is added
}
