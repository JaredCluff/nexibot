...
//!
//! Uses the same Meta webhook infrastructure as Messenger but targets the
//! Instagram Messaging API. Outbound messages are sent to the Instagram
//! Graph API endpoint for the configured business account.
//!
//! Security:
//!   - GET /api/instagram/webhook  — webhook verification (hub.challenge)
//!   - POST /api/instagram/webhook — HMAC-SHA256 signature verified via
//!     `X-Hub-Signature-256` before any processing occurs
//!
//! The Instagram webhook event shape mirrors Messenger (entry → messaging →
//! sender/message), so parsing logic is near-identical.
...
/// Configuration for the Instagram Direct Messages integration.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct InstagramConfig {
    /// Whether the Instagram integration is enabled.
    pub enabled: bool,
    /// Instagram Graph API access token (long-lived page/user token with
    /// `instagram_manage_messages` permission).
    #[serde(default)]
    pub access_token: String,
    /// The Instagram Business Account ID (numeric string, e.g. "17841400123456789").
    /// Used to construct the Send API endpoint.
    #[serde(default)]
    ...
    /// Allowlist of Instagram sender IDs. Empty = allow all when dm_policy is Allowlist.
    #[serde(default)]
    pub allowed_sender_ids: Vec<String>,
    /// Sender IDs with admin privileges (bypass DM policy).
    #[serde(default)]
...
impl Default for InstagramConfig {
    fn default() -> Self {
        Self {
            ...
            instagram_account_id: String::new(),
...
/// Per-sender session for Instagram DM conversations.
pub(crate) struct InstagramChatSession {
    claude_client: ClaudeClient,
    last_activity: Instant,
}

/// Shared state for the Instagram webhook handler.
pub struct InstagramState {
    pub app_state: AppState,
    pub chat_sessions: RwLock<HashMap<String, InstagramChatSession>>,
    pub rate_limiter: Arc<RateLimiter>,
    pub msg_dedup: Mutex<LruCache<String, ()>>,
}

/// Observer for Instagram tool execution flow, including in-channel approvals.
pub(crate) struct InstagramObserver {
    app_state: AppState,
...
impl crate::tool_loop::ToolLoopObserver for InstagramObserver {
    ...
    async fn request_approval(&self, tool_name: &str, reason: &str) -> bool {
        ...
        if !send_instagram_message_checked(&self.app_state, &self.requester_sender_id, &prompt)
            .await
        {
...
pub struct InstagramVerifyQuery {
...
/// The shape mirrors the Messenger webhook payload.
#[derive(Debug, Deserialize)]
struct InstagramWebhookPayload {
    object: Option<String>,
    entry: Option<Vec<InstagramEntry>>,
}

#[derive(Debug, Deserialize)]
struct InstagramEntry {
    messaging: Option<Vec<InstagramEvent>>,
}

#[derive(Debug, Deserialize)]
struct InstagramEvent {
    sender: Option<InstagramId>,
    #[allow(dead_code)]
    recipient: Option<InstagramId>,
    message: Option<InstagramMessage>,
}
...
struct InstagramMessage {
    mid: Option<String>,
    text: Option<String>,
    /// True if this is an echo of the account's own outbound message.
    #[serde(default)]
    is_echo: bool,
}
...
/// Handle Meta webhook verification for Instagram (GET /api/instagram/webhook).
pub async fn instagram_verify_handler(
    AxumState(state): AxumState<Arc<InstagramState>>,
    Query(query): Query<InstagramVerifyQuery>,
    ...
    let (verify_token, enabled) = {
        ...
        (
            state
                .app_state
                .key_interceptor
                .restore_config_string(&config.instagram.verify_token),
    ...
    if verify_token.is_empty() {
        warn!("[INSTAGRAM] verify_token not configured — rejecting verification request");
            ...
            .unwrap_or(false)
    {
        ...
        warn!("[INSTAGRAM] Webhook verification failed");
        Err((StatusCode::FORBIDDEN, "Verification failed".to_string()))
    }
}

/// Handle incoming Instagram webhook events (POST /api/instagram/webhook).
///
...
pub async fn instagram_webhook_handler(
    ...
    let (app_secret, enabled) = {
        ...
        (
            state
                .app_state
                .key_interceptor
                .restore_config_string(&config.instagram.app_secret),
            config.instagram.enabled,
        )
    };
    ...
    if app_secret.is_empty() {
        warn!("[INSTAGRAM] app_secret not configured — rejecting webhook");
        return StatusCode::UNAUTHORIZED;
    }

    // Signature verification MUST happen before parsing the body
    if !verify_meta_signature(&body, &app_secret, &headers) {
        warn!("[INSTAGRAM] Invalid X-Hub-Signature-256 — rejecting request");
        return StatusCode::UNAUTHORIZED;
    }
    ...
    let payload: serde_json::Value = match serde_json::from_slice(&body) {
        Ok(v) => v,
        Err(e) => {
            warn!("[INSTAGRAM] Failed to parse webhook body: {}", e);
    ...
    tokio::spawn(async move {
        if let Err(e) = process_instagram_payload(state, payload).await {
            error!("[INSTAGRAM] Error processing payload: {}", e);
        }
    });
...
/// Parse and dispatch an Instagram webhook payload.
async fn process_instagram_payload(
    state: Arc<InstagramState>,
    payload: serde_json::Value,
) -> Result<(), String> {
    let webhook: InstagramWebhookPayload = serde_json::from_value(payload)
        .map_err(|e| format!("Failed to parse Instagram payload: {}", e))?;

    // Only handle instagram events
    if webhook.object.as_deref() != Some("instagram") {
        return Ok(());
    }
    ...
    for entry in webhook.entry.iter().flatten() {
        for event in entry.messaging.iter().flatten() {
            ...
            // Skip echoes (the account's own outbound messages)
            if message.is_echo {
                continue;
            }
            ...
            let (allowed_sender_ids, admin_sender_ids, dm_policy, enabled) = {
                let config = state.app_state.config.read().await;
                (
                    config.instagram.allowed_sender_ids.clone(),
                    config.instagram.admin_sender_ids.clone(),
                    config.instagram.dm_policy,
                    config.instagram.enabled,
                )
            };
            ...
            if !is_admin {
                match dm_policy {
                    crate::pairing::DmPolicy::Allowlist => {
                        ...
                        {
                            info!(
                                "[INSTAGRAM] Ignoring message from unauthorized sender: {}",
                                sender_id
                            );
                            continue;
                        }
                    ...
                    crate::pairing::DmPolicy::Pairing => {
                        ...
                        if !is_allowed {
                            ...
                            match pairing_mgr.create_pairing_request("instagram", sender_id, None) {
                                Ok(code) => {
                                    drop(pairing_mgr);
                                    send_instagram_message(
                                        &state.app_state,
            ...
            let dedup_key = message.mid.clone().unwrap_or_default();
            if !dedup_key.is_empty() {
                let mut dedup = state.msg_dedup.lock().await;
                if dedup.put(dedup_key.clone(), ()).is_some() {
                    debug!("[INSTAGRAM] Dropping duplicate message: {}", dedup_key);
                    continue;
                }
            }

            // --- Rate limiting ---
            let rate_key = format!("instagram:{}", sender_id);
            if state.rate_limiter.check(&rate_key).is_err() {
                warn!("[INSTAGRAM] Rate limit exceeded for sender: {}", sender_id);
                continue;
            }
            ...
            if matches!(
                ...
                if let Some(approval_tx) = approval_tx {
                    ...
                    send_instagram_message(&state.app_state, sender_id, reply).await;
                    continue;
                }
            }
            ...
            tokio::spawn(async move {
                handle_instagram_text_message(&state_clone, &sender_owned, &text_owned).await;
            });
        }
    }
...
/// Route an Instagram DM text message through the Claude pipeline.
async fn handle_instagram_text_message(state: &InstagramState, sender_id: &str, text: &str) {
    ...
    let message = IncomingMessage {
        text: text.to_string(),
        channel: ChannelSource::Instagram {
    ...
    let has_send_config = {
        ...
        !config.instagram.access_token.trim().is_empty()
            && !config.instagram.instagram_account_id.trim().is_empty()
    };

    let observer = InstagramObserver::new(
        app_state.clone(),
        sender_id.to_string(),
        has_send_config,
        app_state.instagram_pending_approvals.clone(),
    );
    let options = RouteOptions {
        claude_client: &claude_client,
        overrides: SessionOverrides::default(),
        loop_config: ToolLoopConfig::instagram(sender_id.to_string()),
        observer: &observer,
        streaming: false,
        window: None,
    ...
    match router::route_message(&message, options, app_state).await {
        Ok(routed) => {
            ...
            if response.is_empty() {
                send_instagram_message(app_state, sender_id, "(No response)").await;
            } else {
                // Instagram DMs have a 1000-character limit per message
                for chunk in router::split_message(&response, 1000) {
                    send_instagram_message(app_state, sender_id, &chunk).await;
                }
            }
        }
        Err(RouterError::Blocked(msg)) => {
            send_instagram_message(app_state, sender_id, &msg).await;
        }
        Err(e) => {
            send_instagram_message(app_state, sender_id, &format!("Error: {}", e)).await;
        }
    }
}

/// Send a text message to an Instagram DM recipient via the Meta Graph API.
pub async fn send_instagram_message(app_state: &AppState, recipient_id: &str, text: &str) {
    let _ = send_instagram_message_checked(app_state, recipient_id, text).await;
}

async fn send_instagram_message_checked(
    app_state: &AppState,
    recipient_id: &str,
    text: &str,
) -> bool {
    let (access_token, instagram_account_id) = {
        ...
        (
            app_state
                .key_interceptor
                .restore_config_string(&config.instagram.access_token),
            config.instagram.instagram_account_id.clone(),
        )
    };

    if access_token.is_empty() || instagram_account_id.is_empty() {
        error!("[INSTAGRAM] Cannot send: access_token or instagram_account_id not configured");
        return false;
    }
        ...
        .await
    {
        Ok(resp) if resp.status().is_success() => {
            info!("[INSTAGRAM] Message sent to {}", recipient_id);
            true
        }
        Ok(resp) => {
            let status = resp.status();
            let body_text = resp.text().await.unwrap_or_default();
            error!("[INSTAGRAM] Send failed ({}): {}", status, body_text);
            false
        }
        Err(e) => {
            error!("[INSTAGRAM] HTTP error sending message: {}", e);
            false
        }
    }
}
...
/// Maximum number of concurrent Instagram DM sessions.
const MAX_CHANNEL_SESSIONS: usize = 1000;

/// Periodically clean up stale Instagram sessions (>24h inactive).
pub async fn session_cleanup_loop(state: Arc<InstagramState>) {
    ...
    loop {
        ...
        if removed > 0 {
            info!(
                "[INSTAGRAM] Cleaned up {} stale sessions ({} remaining)",
                removed,
                sessions.len()
            );
        }
        ...
        if sessions.len() > MAX_CHANNEL_SESSIONS {
            ...
            info!(
                "[INSTAGRAM] Evicted {} oldest sessions to enforce cap (now {})",
                evict_count,
                sessions.len()
            );
        }
...
/// Start background tasks for the Instagram integration.
///
/// The webhook handlers are mounted externally (api_server.rs / webhooks.rs).
pub async fn start_instagram(app_state: AppState) -> Result<(), String> {
    let enabled = {
        let config = app_state.config.read().await;
        config.instagram.enabled
    };
    ...
    info!("[INSTAGRAM] Integration enabled — webhook handlers should be mounted at /api/instagram/webhook");
    Ok(())
}
